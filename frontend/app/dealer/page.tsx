...
export default function DealerDashboard() {
  ...
  useEffect(() => {
    if (!authLoading && !isAuthenticated) {
      console.log('❌ Пользователь не аутентифицирован, перенаправляем на /unauthorized')
      router.push('/unauthorized')
    }
  }, [isAuthenticated, authLoading, router])

  // Если проверяем аутентификацию, показываем загрузку
  if (authLoading) {
    return (
  ...
  // Если не аутентифицирован, не показываем контент
  if (!isAuthenticated) {
    return null
  }
  ...
  const fetchCars = async () => {
    ...
    try {
      ...
      const transformedCars = (data.results || []).map(transformCarFromAPI)
                      ...
                      <CardContent className="p-3 lg:p-4">
                        ...
                        <div className="flex flex-col sm:flex-row gap-2">
                          <Button 
                            size="sm" 
                            variant="outline"
                            onClick={() => handleStatusChange(car.id.toString(), car.status === 'published' ? 'draft' : 'published')}
                            className="text-xs"
                          >
                      ...
                      <CardContent className="p-3 lg:p-4">
                        ...
                        <div className="flex flex-col sm:flex-row gap-2">
                          <Button 
                            size="sm" 
                            variant="outline"
                            onClick={() => handlePartStatusChange(Number(part.id), part.status === 'published' ? 'draft' : 'published')}
                            className="text-xs"
                          >
                          ...
                          <div className="flex gap-2">
                            ...
                            <Button 
                              size="sm" 
                              variant="outline"
                              onClick={() => handleDeletePart(Number(part.id))}
                              className="text-red-600 hover:text-red-700 flex-1"
                            >
                              <Trash2 className="h-3 w-3 lg:h-4 lg:w-4" />
                            </Button>
